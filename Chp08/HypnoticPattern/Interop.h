...
// handles to data intended to share between OpenGL and CUDA
GLuint buffer_obj;
cudaGraphicsResource* resource;

namespace CudaGLInterop
{
	int selectCudaDev(cudaDeviceProp& prop)
	{
		unsigned int cnt;
		int dev;
	...
	void setInterop(int argc, char* argv[])
	{
	...
	void launchCudaKernel()
	{
		...
		uchar4* dev_ptr = getDevicePointer();

		dim3 grid((DIMX + 15) / 16, (DIMY + 15) / 16);
		dim3 threads(16, 16);
		kernel << <grid, threads >> > (dev_ptr);

		// provide synchronization between CUDA and Graphics portions of application
		HANDLE_ERROR(cudaGraphicsUnmapResources(1, &resource, NULL)); /* <-- very important */
	}
	...
	void setGlutMainLoop(void (*keyFunc)(unsigned char, int, int))
	{
