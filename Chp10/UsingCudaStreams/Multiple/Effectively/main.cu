...
int main()
{
    ...
    for (int i = 0; i < streams * (FULL_DATA_SIZE / streams);
        i += MB * streams)
    {
        // pass 1: enqueue H->D copies across all streams
        copyLockedMemAsync<int*>(dev_buffers, host_buffers,
            MB * sizeof(int), cudaMemcpyHostToDevice, cuda_streams,
            0, 0, 2, i, FULL_DATA_SIZE, streams);

        // pass 2: enqueue kernels across all streams
        ...
        // pass 3: enqueue D->H copies across all streams
        copyLockedMemAsync<int*>(dev_buffers, host_buffers,
            MB * sizeof(int), cudaMemcpyDeviceToHost, cuda_streams,
            2, 2, 1, i, FULL_DATA_SIZE, streams);
    }
